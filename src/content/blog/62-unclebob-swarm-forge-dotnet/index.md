...
![Uncle Bob's Agent Pipeline: From Informal Specs to Mutation-Tested .NET Code – by Adrian Bailador Panero](swarm-forge.png)
...
```
You → informal specs (handwritten notes)
         ↓
   [Specifier Agent]  →  hard specs + task list
         ↓  ← you review
   [Specifier Agent]  →  Gherkin per task
         ↓  ← you spot-check
   [Coder Agent]      →  acceptance tests → unit tests → code
         ↓
   [Refactorer Agent] →  complexity ≤ 6, no duplication, property tests
         ↓
   [Architect Agent]  →  language mutation → Gherkin mutation → full suite
         ↓
   back to Specifier / Coder / Refactorer for the next task
```
...
*Questions or suggestions? Open an issue on [GitHub](https://github.com/AdrianBailador/AdrianBailador.github.io/issues).*
